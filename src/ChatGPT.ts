import { Agent, Initializer, LLM } from "@k-apps-io/llm-dsl";
...
export type Options = LLM.Model.Options &
  Omit<ChatCompletionCreateParams, "messages" | "stream" | "model"> & {
...
const determinEncoder = (model: string): Tiktoken => {
  ...
  try {
    ...
    process.emitWarning(
      `myEncoder could not be determined for model: ${model}`
    );
...
const main = (
  ...
  if (typeof content === "string") {
    return myEncoder.encode(content).length;
  } else if (Array.isArray(content)) {
    return content.reduce((total, item) => {
      if (item.type == "text") {
        total += myEncoder.encode(item.text).length;
...
> extends Agent<Options, Prompts, Responses, ToolResults, Locals, Metadata> {
  constructor(
    service: ChatGPTOptions,
    options: Omit<
      Initializer<Options, Prompts, Responses, ToolResults, Locals, Metadata>,
      "llm"
    > = {}
  ) {
    super({
...
export class Service
  implements LLM.Model.Service<Options, Prompts, Responses, ToolResults>
{
  ...
  constructor(options: ChatGPTOptions) {
    ...
    if (typeof options.model !== "string") {
      ...
      this.encoder = determinEncoder(options.model);
    }
  ...
  functionTokens(
    functions: Omit<
      LLM.Tool.Tool<Options, Prompts, Responses, ToolResults>,
      "func"
    >[]
  ): {
    total: number;
